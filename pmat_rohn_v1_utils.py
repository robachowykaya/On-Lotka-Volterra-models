...
def p_matrix_Rohn_v1(A, make_prints):
    """
    A : square matrix
    ...
    if (np.linalg.det(A) < 0) | (np.diag(A) < 0).any(): # added for robustness
        msg = "det < 0 or negative diagonal element"
        return 0, [], msg
    pm = 1
    J = []
    n = A.shape[0]
    ...
    if is_sym_pos_def(A):
        if make_prints:
            msg = "In p_matrix_Rohn_v1: is_sym_pos_def(A)"
        return pm, J, msg
    if is_singular(A - I) | is_singular(A + I):
        pm = -1
        if make_prints:
            msg = "In p_matrix_Rohn_v1: is_singular(A - I) | is_singular(A + I)"
        return pm, J, msg
    C = np.linalg.inv(A - I) @ (A + I)
    R = np.linalg.inv(C)
    if spectral_radius(np.abs(R)) < 1:
        if make_prints:
            msg = "In p_matrix_Rohn_v1: spectral_radius(np.abs(R)) < 1"
        return pm, J, msg
    ...
    if len(x) != 0:
        if make_prints:
            msg = "In p_matrix_Rohn_v1: intervallhull"
        return pm, J, msg
    ...
    if (x != 0).all():
        ...
        J = []
    if make_prints:
        msg = "In p_matrix_Rohn_v1: determines minor negative"
    return pm, J, msg
...
def intervallhull(A, b):
    ...
    Z = []
    Z.append(list(z))
    #print("a list with 1 list element",Z)
    D = []
    while len(Z) != 0:
        #print("enter while intervall hull")
        z = list(random.sample(Z, 1)[0])
        if z in Z:
            Z.remove(z)
            #print("a list with 1 list element less",Z)
            D.append(z)
        ...
        if (xz_floor <= xz_ceilling).all():
            ...
            for j in range(n):
                ...
                #print("new_z", new_z)
                new_z[j] = -new_z[j]
                #print("change of sign", new_z)
                #print("enter for loop intervall hull")
                #print("xz_floor, xz_ceilling, z, new_z\n",xz_floor, xz_ceilling, z, new_z)
                #print("xz_floor[j]*xz_ceilling[j] <= 0",xz_floor[j]*xz_ceilling[j] <= 0)
                #print("new_z not in Z?", new_z, Z)
                #print("new_z not in D?", new_z, D)
                if (xz_floor[j]*xz_ceilling[j] <= 0) & (new_z not in Z) & (new_z not in D):
                    Z.append(new_z)
                    #print("a list with list elements",Z)
