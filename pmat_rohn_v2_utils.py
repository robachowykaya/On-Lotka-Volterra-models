...
def regular_interval_matrix(Ac, Delta, make_prints):
    """  
    """
    msg = ""
    pb = 0
    ...
    if np.linalg.matrix_rank(Ac) < n:
        S = Ac
        if make_prints:
            msg = "In regular_interval_matrix: Singularity of the midpoint matrix 'Ac'"
        return S, orthants_visited, msg, pb
    ...
    if dg >= 1:
        ...
        if make_prints:
            msg = "In regular_interval_matrix: Singularity via diagonal condition"
        return S, orthants_visited, msg, pb
    if beeck_condition(Ac, Delta) < 1:
        S = []
        if make_prints:
            msg = "In regular_interval_matrix: Regularity via Beeck's condition"
        return S, orthants_visited, msg, pb
    S, pb = singularity_det_descent(Ac, Delta)
    if len(S) != 0:
        if make_prints:
            msg = "In regular_interval_matrix: Singularity via steepest determinant descent"
        return S, orthants_visited, msg, pb
    AA = Ac.T @ Ac
    DD = Delta.T @ Delta
    if (np.linalg.matrix_rank(AA) == n) & (beeck_condition(AA, DD) < 1):
        S = []
        if make_prints:
            msg = "In regular_interval_matrix: Regularity  via symmetrization"
        return S, orthants_visited, msg, pb
    rs, S = regularity_suffcond_qz(Ac, Delta)
    if rs == 0:
        if make_prints:
            msg = "In regular_interval_matrix: Singularity as a by-product of 'regularity via two Qz-matrices'"
        return S, orthants_visited, msg, pb
    if rs == 1:
        S = []
        if make_prints:
            msg = "In regular_interval_matrix: Regularity via two Qz-matrices"
        return S, orthants_visited, msg, pb
    eps = 1e-8
    ...
    while len(Z) != 0:
        ...
        if len(S) != 0:
            ...
            if make_prints:
                msg = "In regular_interval_matrix: Singularity via the main algorithm"
            #if do_plot_orthants:
            #    to_plot(yy)
            return S, orthants_visited, msg, pb
        xut = Q @ bc
    ...
    orthants_visited = len(D)
    if make_prints:
        msg = "In regular_interval_matrix: Regularity via the main algorithm"
    #if do_plot_orthants:
    #    to_plot(yy)        
    return S, orthants_visited, msg, pb
...
def p_matrix_Rohn_v2(A, make_prints):
    """
    ...
    if is_sym_pos_def(A):
        if make_prints:
            msg = "In p_matrix_Rohn_v2: is_sym_pos_def(A)"
        return 1, msg
    if (np.linalg.det(A) < 0) | (np.diag(A) < 0).any(): # added for robustness
        if make_prints:
            msg = "p_matrix_Rohn_v2: Negative det or diagonal"
        return 0, msg
    else:
        if np.linalg.matrix_rank(A-I) < n:
            if make_prints:
                msg = "p_matrix_Rohn_v2: Algorithm failed"
            return -1, msg
        else:
            B = np.linalg.inv(A-I) @ (A+I)
            S, nbr_orthants_visited, msg_regising, pb_regising = regular_interval_matrix(B, I, make_prints)
            if pb_regising==1:
                msg = "problem in regising (nbr_iter to find beta)"
                return -1, msg
            if len(S) != 0:
                if make_prints:
                    msg = "p_matrix_Rohn_v2: Not a P-matrix because S singular " + msg_regising
                return 0, msg
            else:
                if make_prints:
                    msg = "p_matrix_Rohn_v2: P-matrix since no S singular " + msg_regising
                return 1, msg